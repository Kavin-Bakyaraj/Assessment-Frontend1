...
const AIGenerate = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "selectedLevel") {
      // Only add the level if it's not already in the list and not the default "Select a level" option
      if (value && !formData.level.some((l) => l.value === value)) {
      ...
      if (value.length <= MAX_SUBTOPIC_LENGTH) {
        setFormData((prevData) => ({
          ...prevData,
          [name]: value,
        }));
  ...
  const removeLevel = (levelValue) => {
    setFormData((prevData) => {
      const updatedLevels = prevData.level.filter((l) => l.value !== levelValue);
      return {
        ...prevData,
        level: updatedLevels,
        selectedLevel: updatedLevels.length === 0 ? "" : prevData.selectedLevel, // Reset selectedLevel if no levels are left
      };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Validate total questions doesn't exceed MAX_TOTAL_QUESTIONS
      if (numQuestions > MAX_TOTAL_QUESTIONS) {
        setErrorMessage(`Total questions cannot exceed ${MAX_TOTAL_QUESTIONS}.`);
        setLoading(false);
        return;
      }
      ...
      if (invalidLevel) {
        ...
        setErrorMessage(`Please specify the number of questions for ${levelLabel}.`);
        setLoading(false);
        return;
      }
      ...
      if (totalQuestionsAllocated !== numQuestions) {
        setErrorMessage(`The sum of questions (${totalQuestionsAllocated}) must equal the total number of questions (${numQuestions}).`);
        setLoading(false);
        return;
      }
      ...
      if (levelExceedingLimit) {
        ...
        setErrorMessage(`${levelLabel} cannot have more than ${MAX_QUESTIONS_PER_LEVEL} questions.`);
        setLoading(false);
        return;
      }
      ...
      // Create the request data object to pass for regeneration
      const formInputs = {
        topic: formData.topic,
        subtopic: formData.subtopic,
  ...
  return (
    <div className="h-screen p-12 py-20 pt-28 "
      style={{
      ...
      <div className="max-w-full mx-auto bg-white p-10 shadow-lg rounded-2xl">
        <h1 className="text-3xl font-bold mb-2 text-[#111933]">Question Generator AI</h1>
        <p className="text-sm mb-6">
          Enter the below details and click generate to generate the questions.
        </p>
        <hr className="mb-12 mt-6 border-1 border-gray-500" />
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-40 px-8 ">
          {/* Topic */}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center">
                        ...
                        <input
                          ...
                          className="w-40 text-sm px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 appearance-none"
                        />
                      </div>
                      <button
                        type="button"
