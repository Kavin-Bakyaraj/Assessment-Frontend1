...
const SectionSelectTestQuestion = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      try {
        ...
        // Automatically select the first test if available
        if (sanitizedTests.length > 0) {
          setSelectedTest(sanitizedTests[0]);
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    // Retrieve existing form data and sections from sessionStorage
    ...
    // Add the selected test questions to the existing selectedQuestions array
    sections[0].selectedQuestions.push(...selectedQuestions);

    // Save the updated sections back to sessionStorage
  ...
  const handleTestSelect = (test) => {
    if (selectedTest && selectedTest.test_id === test.test_id) {
      // Deselect the test if it is already selected
      setSelectedTest(null);
    } else {
      // Select the test
  ...
  return (
    <div className="min-h-screen p-8 py-20 pt-28"
      ...
      <div className="h-14 ml-2 pb-10">
        <div className="flex items-center gap-2 text-[#111933] text-sm">
          <span className="cursor-pointer opacity-60 hover:scale-102 transition-all duration-100e" onClick={() => setIsConfirmModalOpen(true)}>Home</span>
          <span className="flex items-center -space-x-2">
            <ChevronRight size={15} />
            <ChevronRight size={15} />
          </span>
          <span className="cursor-pointer opacity-60 hover:scale-102 transition-all duration-100" onClick={() => navigate('/mcq/combinedDashboard')}>Assessment Overview</span>
          <span className="flex items-center -space-x-2">
            <ChevronRight size={15} />
            <ChevronRight size={15} />
          </span>
          <span className="cursor-pointer opacity-60 hover:scale-102 transition-all duration-100" onClick={() => navigate('/mcq/details')}>Test Configuration</span>
          <span className="flex items-center -space-x-2">
            <ChevronRight size={15} />
            <ChevronRight size={15} />
          </span>
          <span className="cursor-pointer opacity-60 hover:scale-102 transition-all duration-100" onClick={() => window.history.back()}>
            Add Questions
          </span>
          <span className="flex items-center -space-x-2">
            <ChevronRight size={15} />
      ...
      <div className="max-w-8xl bg-white border rounded-lg mx-auto">
              ...
              {tests.map((test) => (
                ...
                >
                  <input
                    ...
                    className="mr-2"
                  />
                  <h3 className="font-medium flex-1">{test.test_name}</h3>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className='w-full flex justify-between p-8'>
          <button
            ...
            className={`mt-4 py-2 px-9 rounded-lg text-sm bg-[#111933] text-white border text-[#00975] hover:bg-opacity-80 ${selectedQuestions.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          <button
            ...
            className="mt-4 py-2 px-9 rounded-lg text-sm bg-[#111933] text-white border text-[#00975] hover:bg-opacity-80 ml-4"
          >
            Save Selected Test <MdDownloading className=' size-5 inline-flex ml-2'/>
          </button>
        </div>
      </div>
...
export default SectionSelectTestQuestion;
