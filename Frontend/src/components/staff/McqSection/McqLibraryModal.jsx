...
const McqLibrary = ({ onClose, onQuestionsSelected }) => {
    ...
    const fetchQuestions = async () => {
      ...
      try {
        ...
        const sanitizedQuestions = response.data.questions.map(question => ({
          ...question,
          // Add this check to handle both string and array formats of tags
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 backdrop-filter  backdrop-blur-sm w-screen h-screen z-50">
      <div className="w-full max-w-[90%] h-full max-h-[90%] bg-white rounded-lg p-4 overflow-hidden relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 bg-transparent rounded-full hover:bg-gray-200 duration-300"
        >
            ...
            <div className="w-2/4 p-2">
              <div className="flex items-center mb-4">
                <button
                  ...
                  className="py-1 px-4 bg-white border-2 border-gray-300  font-semibold text-[#111933] rounded-xl duration-300 hover:bg-gray-100 text-sm"
                >
                  ...
                  <div
                    ...
                    className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                      selectedQuestions.includes(indexOfFirstQuestion + index)
                        ? 'border-[#111933] bg-blue-50'
                        : 'border-gray-300 hover:border-gray-500 duration-300'
                    }`}
